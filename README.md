...
**frozen archive** with no application server, no build hooks, and no hosted
...
| `.github/workflows/` | `build.yml` — Hugo build + GitHub Pages deploy. |
...
Prerequisites: **Hugo extended ≥ 0.161**.
...
A few malformed `indexKey` values from the source (stray spaces, `*`) were
normalised into clean URL slugs when the snapshot was taken; the original key
stays visible on each person page.
